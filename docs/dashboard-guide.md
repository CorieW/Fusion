...
On mobile viewports (`<=768px`), the sidebar is not rendered even when the default-on setting is enabled. The Header's **New Task** action is the rightmost header control and opens the existing full form from any active project view. The bottom `MobileNavBar` provides separate **Board** and **List** destinations, while the Planning column keeps its inline quick-entry composer. **Artifacts** and **Recommendations** are available either as configured primary tabs or in the **More** sheet, with the same project-scoped new-item counters in both placements. Mobile-only More-sheet entries remain available for compact tools such as Git Manager, Terminal, Files, and **Import from GitHub**.

The default mobile bar is **Dashboard → Board → List → Chat → Agents → Workflows → Memory → More**, matching desktop order. Memory appears when its feature is enabled. **Missions** and **Mailbox** live in **Other**, both in the sidebar and mobile More. Previously saved defaults upgrade automatically; deliberately customized shortcuts retain their selected destinations and order.

Mobile **More** follows the desktop sidebar hierarchy: **Dashboard** stands alone, **Tasks** contains Board/List, **AI** contains Chat/Agents/Workflows/Memory, and **Other** alphabetizes the remaining destinations, plugins, and mobile tools. Tasks and AI start expanded; Other starts collapsed. Only destinations not already pinned in the bottom bar appear in More, and empty groups are omitted. **Settings** remains outside the collapsible groups when it is not pinned. The dedicated **List** shortcut sits beside **Board**; custom shortcut order is otherwise preserved. On narrow screens, shortcuts form balanced rows with readable wrapping labels and at least 44px touch targets instead of squeezing into a single row. The footer and content spacing follow the complete navigation height, including after rotation or closing a modal. Collapsed groups retain an attention dot for pending input or unread items. On compact screens, stacked warning banners scroll within a bounded area rather than consuming the page. Mobile Settings scrolls its section picker and fields together so both remain reachable above the footer.
...
- Every persisted message has a quote control. Quoting an agent reply includes a short excerpt, a link identifying the complete source message, and the author's mention. The source is retrieved even when it falls outside the recent history window.
- Mentioned agents receive complete messages selected within an estimated token budget derived from their model's context window, with capacity reserved for instructions, tools, and the answer. The latest instruction is supplied in full. The preceding review, quoted messages, original request, and replies already made to the current instruction take priority. Messages that cannot fit are explicitly marked for retrieval instead of being silently cut off.
- Every model-loop participant can use `fn_chat_thread_read` and `fn_chat_thread_search` to recover earlier messages. Reads page through complete message text, saved tool evidence, and attachments, including text beyond the initial attachment inline limit. Search has a continuation offset so older history remains reachable. These tools are bound to the current conversation and project.
- `fn_chat_context_update` maintains source-linked objectives, constraints, decisions, findings, actions, and questions. Agents reuse an entry's ID when updating or resolving it. Accepted decisions and constraints require a user-message source; notes remain agent interpretations that can be checked against the original messages. Each successful reply persists a working-note checkpoint, so subsequent agents inherit it and message rewind removes discarded checkpoints.
- An explicit sequence such as `@Coder fix both findings. Then @Reviewer review the result.` runs in mention order. Repeated mentions of the same agent are separate steps, so coder-to-reviewer-to-coder sequences retain the final assignment. Each participant must report `completed` with evidence through `fn_chat_handoff` before a dependent agent starts. A blocked, failed, skipped, cancelled, or unreported step stops the chain. The next agent receives the persisted reply and reported evidence and must verify the work. Group mentions without a sequencing word remain independent responses.
- Expand **Conversation context** beneath a reply to see how many complete earlier messages and working notes were supplied, and whether history or required references needed retrieval. This reports prompt coverage, not proof that the agent read omitted material. Returning to the ordinary conversation model also supplies intervening mentioned-agent replies that are absent from its provider session file.
...
- Retained room APIs use the same complete-message context builder, scoped retrieval, working notes, and ordered handoffs as Direct mentions. All participants in an ordered handoff must be available room members; ambient members do not join that sequence. The legacy recent-message and summary-character settings no longer truncate the transcript; `chatRoomCompactionFetchLimit` controls the initial history fetch, while tools can retrieve older messages.
