...
describe("project configuration duplication", () => {
  it.each([false, true])("copies a project with workflows=%s and never reads tasks/history", async populated => {
    const workflow = { id: "WF-old", name: "Process", description: "Custom", ir: { version: "v2", nodes: [], edges: [], columns: [] }, layout: {} };
    const source = { getFusionDir: () => "/source/.fusion", getRootDir: () => "/source", getAsyncLayer: () => undefined, listWorkflowDefinitions: vi.fn(async () => populated ? [workflow] : []), getSettingsByScope: async () => ({ project: { defaultWorkflowId: populated ? "WF-old" : "", maxConcurrent: 3, enginePaused: false } }), listTasks: vi.fn() };
    const target = { getFusionDir: () => "/target/.fusion", getRootDir: () => "/target", getAsyncLayer: () => undefined, updateSettings: vi.fn(async () => {}), createWorkflowDefinition: vi.fn(async () => ({ id: "WF-new" })), updateWorkflowDefinition: vi.fn(async () => {}) };
