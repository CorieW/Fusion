...
/*
FNXC:DashboardTheming 2026-09-07-04:09:
Golden Darkness is valid only when persistence, first-paint validators, selector metadata, tokens, and globally resolvable previews agree. This source contract catches a partial registration before a saved operator preference can flash or fall back.
*/
describe("Golden Darkness color theme", () => {
  ...
  it("keeps persisted, selector, and first-paint registries in exact order", () => {
    ...
    expect(CORE_COLOR_THEMES.filter((theme) => theme === "golden-darkness")).toHaveLength(1);
    expect(DASHBOARD_COLOR_THEMES).toContainEqual({
      value: "golden-darkness",
      label: "Golden Darkness",
      className: "theme-swatch-golden-darkness",
    });
  ...
  it("defines complete readable dark and light Golden Darkness token blocks", () => {
    const darkBlock = extractSelectorBlock(themeData, '[data-color-theme="golden-darkness"]');
    const lightBlock = extractSelectorBlock(themeData, '[data-color-theme="golden-darkness"][data-theme="light"]');
    ...
    expect(darkBlock).toContain("--accent: #d4af37;");
    expect(lightBlock).toContain("--bg: #f4f0e8;");
    expect(lightBlock).toContain("--accent: #8b6b16;");
  });

  it("uses mode-specific global Golden Darkness preview properties for an unselected swatch", () => {
    const darkGlobals = extractSelectorBlock(themeData, ":root");
    const lightGlobals = extractSelectorBlock(themeData, '[data-theme="light"]');
    const darkSwatch = extractSelectorBlock(themeSelector, ".theme-swatch-golden-darkness");
    const lightSwatch = extractSelectorBlock(themeSelector, '[data-theme="light"] .theme-swatch-golden-darkness');

    for (const block of [darkGlobals, lightGlobals]) {
      for (const sample of [1, 2, 3, 4]) expect(block).toContain(`--golden-darkness-swatch-sample-${sample}:`);
    }
    for (const block of [darkSwatch, lightSwatch]) {
      for (const sample of [1, 2, 3, 4]) {
        expect(block).toContain(`--swatch-sample-${sample}: var(--golden-darkness-swatch-sample-${sample});`);
      }
  ...
  it("lists Golden Darkness in the published color-theme settings catalog", () => {
    const colorThemeRow = settingsReference.split("\n").find((line) => line.startsWith("| `colorTheme` |"));
    expect(colorThemeRow).toContain("Golden Darkness");
  });
});
