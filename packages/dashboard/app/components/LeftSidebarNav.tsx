...
function readStoredGroups(): Record<SidebarGroupId, boolean> {
  try {
    const saved = JSON.parse(window.localStorage.getItem(LEFT_SIDEBAR_GROUPS_STORAGE_KEY) ?? "null");
    return Object.fromEntries(Object.entries(DEFAULT_GROUPS).map(([id, initial]) => [id, typeof saved?.[id] === "boolean" ? saved[id] : initial])) as Record<SidebarGroupId, boolean>;
...
export function LeftSidebarNav({
  ...
  const [expandedGroups, setExpandedGroups] = useState(readStoredGroups);
  const groupIdPrefix = useId();
  const toggleGroup = (id: SidebarGroupId) => {
    setExpandedGroups(current => {
      const next = { ...current, [id]: !current[id] };
      try { window.localStorage.setItem(LEFT_SIDEBAR_GROUPS_STORAGE_KEY, JSON.stringify(next)); } catch { /* FNXC:SidebarGroups 2026-09-07-02:52: Navigation remains usable when browser storage is unavailable. */ }
      return next;
    });
  };
  /*
      ...
      <nav className="left-sidebar-nav__list" aria-label={t("nav.primaryNavAriaLabel", "Primary navigation")}>
        <div className="left-sidebar-nav__section">
          ...
          {groups.filter(group => group.entries.length > 0).map(group => {
            ...
            return (
              <div className="left-sidebar-nav__group" key={group.id}>
                {!isCollapsed && (
                  <button
                    type="button"
                    className={"left-sidebar-nav__group-toggle" + (active ? " left-sidebar-nav__group-toggle--active" : "")}
